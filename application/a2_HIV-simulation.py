...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(300):
        aveInTime = 0
        for item in tot:
            aveInTime = aveInTime + tot[item][i]
        aveInTime = float(aveInTime)/numTrials
        average.append(aveInTime)    

    pylab.plot(average, label = 'Virus Growth')
    pylab.xlabel('Time Steps')
    pylab.ylabel('Average Virus Particles')
    pylab.title('Production of Viruses')
    pylab.legend()
    pylab.show()
            ...
            if random.random() <= self.maxBirthProb * (1 - popDensity):
                for drug in self.resistances:
                    if random.random() <= self.getMutProb():
                        self.resistances[drug] = not self.resistances
