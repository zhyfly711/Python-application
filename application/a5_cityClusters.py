...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 25 21:29:12 2015

@author: zhyfly711
"""

'''This program is to merge different cities into groups based on the their
similarities of weather, such as temperature and precipitation. The whole
designing is using the agglomerative hierarchical clustering algorithm'''
    ...
    def averageLinkageDist(self, other):
        ...
        for selfPoint in self.points:
            for otherPoint in other.points:
                dist = selfPoint.distance(otherPoint)
                summation += dist
        numOfPairs = (len(self.points)) * (len(other.points))
        return summation/numOfPairs
...
class ClusterSet(object):
    ...
    def mergeOne(self, linkage):
        ...
        shortest = self.findClosest(linkage)
        c1 = shortest[0]
        c2 = shortest[1]
...
def readCityData(fName, scale = False):
    """Assumes scale is a Boolean.  If True, features are scaled. Note: file
    shoule be a text file containing information about various cities. The 
    first few lines starting with the hash (#) represent the column titles of 
    the data available. The lines without the hash represent the actual data, 
    separated by a comma -- in this case, cities and their average temperature
    in Jan, average temperature in April, average temperature in July, average
    temperature in October, annual precipitation in inches, and the number of 
    days of precipitation in a year."""
...
def test():
    points = buildCityPoints('C:\Users\zhyfly711\Desktop\cityTemps.txt', False)
